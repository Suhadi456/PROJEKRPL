...
$total_bayar   = $koneksi->query("SELECT COALESCE(SUM(py.jumlah_bayar),0) as c FROM pembayaran py JOIN pemesanan pm ON py.id_pemesanan=pm.id WHERE pm.id_user=$uid")->fetch_assoc()['c'] ?? 0;
...
<head>
    ...
    <style>
        ...
        .stat-card .value { font-size: 2rem; font-weight: 800; color: #d4af37; }
...
<nav class="navbar navbar-light bg-white shadow-sm px-4" style="border-bottom:2px solid #f5eec8">
    ...
    <div class="d-flex align-items-center gap-3">
        ...
        <a href="../logout.php" class="btn btn-outline-warning btn-sm" onclick="return confirm('Keluar?')">Keluar</a>
    </div>
</nav>
...
<div class="row">
    <div class="col-md-2 sidebar-col shadow-sm pt-3" style="min-height:100vh;background:linear-gradient(180deg,#f9e45b,#d4af37)">
        ...
        <hr style="border-color:rgba(62,42,0,.2);margin:12px 8px">
        <a href="../logout.php" class="sidebar-link" onclick="return confirm('Yakin keluar?')" style="color:#c62828">🚪 Keluar</a>
    </div>
    <div class="col-md-10 p-4">
        <h4 class="fw-bold mb-1" style="color:#d4af37">Selamat Datang, <?= htmlspecialchars(explode(' ', $_SESSION['nama'])[0]) ?>! 👋</h4>
        <p class="text-muted mb-4">Tersedia <strong><?= $sapi_siap ?></strong> sapi siap jual saat ini</p>
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-6 col-md-3">
                <div class="stat-card"><div class="value" style="color:#f57f17"><?= $pesan_pending ?></div><div class="label">Menunggu Konfirmasi</div></div>
            </div>
            <div class="col-6 col-md-3">
        ...
        <!-- CTA Katalog + Riwayat Pembayaran -->
        <div class="row g-3 mb-4">
            <div class="col-md-8">
                <div class="card border-0 shadow-sm h-100" style="background:linear-gradient(135deg,#f9e45b,#d4af37);border-radius:12px">
                    <div class="card-body p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h5 class="fw-bold mb-1" style="color:#3e2a00">🐄 <?= $sapi_siap ?> Sapi Siap Jual</h5>
                            <p class="mb-0" style="color:rgba(62,42,0,.7)">Temukan sapi terbaik untuk kebutuhan Anda</p>
                        </div>
                        <a href="sapi.php" class="btn btn-light fw-bold px-4" style="color:#3e2a00">Lihat Katalog →</a>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card border-0 shadow-sm h-100" style="background:#fff9c4;border-radius:12px;border-left:4px solid #d4af37">
                    <div class="card-body p-4 d-flex flex-column justify-content-between">
                        <div>
                            <div class="fw-bold mb-1" style="color:#8a6d00">💳 Total Pembayaran Anda</div>
                            <div style="font-size:1.4rem;font-weight:800;color:#d4af37"><?= formatRupiah($total_bayar) ?></div>
                        </div>
                        <a href="pembayaran.php" class="btn btn-warning fw-bold mt-3 w-100">Riwayat Pembayaran →</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white fw-bold d-flex justify-content-between align-items-center">
                ...
                <a href="pemesanan.php" class="btn btn-sm btn-outline-warning">Lihat Semua</a>
            </div>
            <div class="table-responsive">
                <table class="table mb-0">
                    <thead class="table-light">
                        <tr><th>Sapi</th><th>Tanggal</th><th>Harga Jual</th><th>Status</th><th>Aksi</th></tr>
                    </thead>
                    <tbody>
                    <?php if ($recent_pesan && $recent_pesan->num_rows > 0):
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="pembayaran.php?id_pesan=<?= $p['id'] ?>" class="btn btn-sm btn-outline-warning" style="font-size:.78rem">💳 Riwayat Pembayaran</a>
                                <?php else: ?>
                    ...
                    <?php endwhile; else: ?>
                        <tr><td colspan="5" class="text-center text-muted py-4">Belum ada pemesanan. <a href="sapi.php" style="color:#d4af37">Pesan sapi sekarang →</a></td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
