        ...
        if (!$id_sapi || !$nama || !$hp || !$tgl || !$harga_jual) {
            ...
            if (!$sapi) {
                ...
                if ($edit_id) {
                    ...
                    $cek_pesan = $koneksi->query("SELECT id FROM pemesanan WHERE id_sapi=$id_sapi AND status IN ('pending','dp') LIMIT 1");
                    if ($cek_pesan->num_rows > 0) {
                        ...
                        // Jika ada DP, simpan ke pembayaran
                        if ($dp > 0) {
...
<div class="card">
    ...
    <div class="table-responsive">
        <table class="data-table">
            <thead>
                <tr>
                    <th>#</th><th>Kode Sapi</th><th>Pembeli</th><th>No HP</th>
                    <th>Tgl Pesan</th><th>Harga Jual</th><th>DP</th><th>Total Bayar</th><th>Status</th><th>Aksi</th>
                </tr>
            </thead>
            <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><?= formatRupiah($r['dp']) ?></td>
                    <td>
                        <?= formatRupiah($r['total_bayar']) ?>
                        <?php if ($r['status']!=='lunas'): ?>
                        <br><small class="text-muted">Sisa: <?= formatRupiah($sisa) ?></small>
                        <?php endif; ?>
                    </td>
                    ...
                    <td>
                        <div class="d-flex gap-2">
                            <?php if ($r['status'] !== 'lunas'): ?>
                            <a href="index.php?page=pembayaran&id_pesan=<?= $r['id'] ?>" class="btn btn-primary btn-sm">💳 Bayar</a>
                            <a href="index.php?page=pemesanan&aksi=edit&id=<?= $r['id'] ?>" class="btn btn-warning btn-sm">✏️</a>
                            ...
                            <?php else: ?>
                            <span class="text-muted" style="font-size:.85rem">✅ Lunas</span>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
            ...
            <div class="modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="harga_jual" id="hargaJual" class="form-control" min="1" required>
                    </div>
                    <div class="form-group">
...
<script>
// Auto-fill harga jual dari harga beli sapi
document.addEventListener('DOMContentLoaded', function() {
    const sel = document.querySelector('[name="id_sapi"]');
    if (sel) {
        sel.addEventListener('change', function() {
            const opt = this.options[this.selectedIndex];
            const h = opt.getAttribute('data-harga');
            if (h) document.getElementById('hargaJual').value = h;
        });
    }
});
</script>
